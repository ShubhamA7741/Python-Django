...
# rooms =[
#     {'id':1,'name':'welcome to django'},
#     {'id':2,'name':'Frontend'},
#     {'id':3,'name':'Design with django'}
# ]
...
def second(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('second', pk=room.id)
...
def deleteRoom(request, pk):
    ...
    if request.method == 'POST':
        room.delete()
        return redirect('first')
    return render(request, 'delete.html', {'odj':room})
...
def deleteMessage(request, pk):
    ...
    if request.method == 'POST':
        message.delete()
        return redirect('first')
    return render(request, 'delete.html', {'odj':message})
...
def activityPage(request):
    room_messages = Massage.objects.all()
